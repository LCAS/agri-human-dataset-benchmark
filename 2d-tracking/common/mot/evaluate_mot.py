...
class EvaluationConfig:
    """Normalized MOT evaluation settings after YAML loading and CLI overrides."""
...
def resolve_path(value: Optional[Any]) -> Optional[Path]:
...
def load_config(path: Path) -> EvaluationConfig:
    """Load one evaluation YAML file into the internal config dataclass."""
    ...
    return EvaluationConfig(
        gt_mot=resolve_path(gt_mot),
        pred_mot=resolve_path(pred_mot),
        iou_threshold=float(raw.get("iou_threshold", 0.5)),
        output_json=resolve_path(raw.get("output_json")),
        output_csv=resolve_path(raw.get("output_csv")),
        metrics=[str(metric) for metric in metrics],
    )


def evaluate_mot(cfg: EvaluationConfig):
    """Compute MOT metrics for one prediction file against one GT file."""
...
def write_outputs(summary, cfg: EvaluationConfig) -> None:
    """Persist evaluation results in CSV and/or JSON form when requested."""
...
def parse_args(default_config: Path) -> argparse.Namespace:
    """Define the shared CLI for MOT evaluation."""

    parser = argparse.ArgumentParser(
        description="Evaluate a MOT prediction file against ground truth.",
        epilog=(
            "Examples:\n"
            "  python .../evaluate_mot.py --config configs/evaluation/default.yaml\n"
            "  python .../evaluate_mot.py --gt reports/runs/example/ground_truth.txt "
...
def merge_cli_overrides(cfg: EvaluationConfig, args: argparse.Namespace) -> EvaluationConfig:
    ...
    if args.gt is not None:
        updates["gt_mot"] = resolve_path(args.gt)
    if args.pred is not None:
        updates["pred_mot"] = resolve_path(args.pred)
    if args.iou_threshold is not None:
        updates["iou_threshold"] = float(args.iou_threshold)
    if args.out_json is not None:
        updates["output_json"] = resolve_path(args.out_json)
    if args.out_csv is not None:
        updates["output_csv"] = resolve_path(args.out_csv)
    return replace(cfg, **updates)


def main(default_config: Path) -> None:
    """CLI entrypoint for benchmark wrappers that supply their default config."""
...
if __name__ == "__main__":
    raise SystemExit("Use a benchmark-specific wrapper so the correct default config is applied.")
